...
serve(async (req) => {
  ...
  try {
    ...
    // Get the winery owner first (needed for both subscription filtering and notification record)
    const { data: winery, error: wineryError } = await supabaseClient
      .from('wineries')
      .select('owner_id')
      .eq('id', payload.wineryId)
      .single()

    if (wineryError) {
      console.log('⚠️ Could not fetch winery owner:', wineryError)
    }

    const ownerId = winery?.owner_id
    console.log(`🔍 Winery owner ID: ${ownerId}`)
    ...
    if (payload.targetSpecificUsers && payload.targetSpecificUsers.length > 0) {
      ...
      console.log(`🔍 Querying subscribers for winery: ${payload.wineryId}`)
      ...
      // Exclude the winery owner from getting their own notifications
      if (ownerId) {
    ...
    // Create notification record in database (using winery data fetched earlier)
    const { data: notificationRecord, error: notificationError } = await supabaseClient
      .from('notifications')
      .insert({
        winery_id: payload.wineryId,
        sender_id: winery?.owner_id || payload.wineryId, // Use owner_id if available, fallback to wineryId
        ...
        const message = {
          message: {
            ...
            data: {
              ...
              clickAction: payload.wineId ? 'OPEN_WINE_DETAIL' : 'OPEN_WINERY_DETAIL'
            },
            android: {
              notification: {
